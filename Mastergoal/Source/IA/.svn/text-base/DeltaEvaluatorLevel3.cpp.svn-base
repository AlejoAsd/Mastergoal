#include "stdafx.h"
#include "DeltaEvaluatorLevel3.h"
...
namespace AI
{
	DeltaEvaluatorLevel3::DeltaEvaluatorLevel3() : DeltaEvaluator()
	{
	...
	DeltaEvaluatorLevel3::DeltaEvaluatorLevel3(float weights[]) : DeltaEvaluator()
	{
		for (int i = 0; i < N_FEATURES; i++)
		{
	...
	void DeltaEvaluatorLevel3::SetWeights(const float *weights)
	{
		for (int i = 0; i < N_FEATURES; i++)
		{
			this->weights[i] = weights[i];
	...
	float DeltaEvaluatorLevel3::Evaluate(const State& before, const State& after, const Play& play)
	{
	...
	void DeltaEvaluatorLevel3::CalculateDeltas(const State& before, const State& after, const Play& play)
	{
		//MOVED_TOWARDS_BALL
	...
	int DeltaEvaluatorLevel3::ReadWeights(ifstream& file)
	{
		float w = 0;
		int i;
	...
	int DeltaEvaluatorLevel3::MinDistToBall(const Board& board, ORIENTATION team)
	{
